...
def new_client(
    ...
    logger.warning("httpx_compat TRACE: new_client called, proxies=%s", proxies)
    limit = httpx.Limits(
    ...
    if not proxies:
        # curl_cffi with Chrome TLS impersonation (bypasses fingerprint-based CAPTCHA)
        try:
            from httpx_compat import AsyncClient

            logger.warning("httpx_compat: creating cffi client for %d proxies: %s", len(proxies), dict(proxies))
            return AsyncClient(
                impersonate="chrome",
                verify=verify,
                http2=enable_http2,
            ...
            logger.warning("curl_cffi client failed (%s), falling back to httpx", e)
