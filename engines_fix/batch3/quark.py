...
# FlareSolverr instance - vt.sm.cn (images API) rejects direct TLS from VPN IPs
...
def request(query, params):
    ...
    # vt.sm.cn (images API) rejects direct TLS handshakes from datacenter/VPN
    # IPs - route it through FlareSolverr (headless browser)
    if quark_category == 'images':
        ...
        params['data'] = dumps(
            {
                'cmd': 'request.get',
                'url': f"{query_url}?{urlencode(query_params)}",
                'maxTimeout': 25000,
                'session': 'quark_images',
            }
        )
    else:
...
def response(resp):
    results = []
    if quark_category == 'images':
        # FlareSolverr wraps the JSON body in an HTML <pre> tag - unwrap it
        text = resp.json()['solution']['response']
    ...
    # sanitize: drop results without absolute URLs and fix protocol-relative
    # URLs (//...) - invalid urls crash the result merging
    def _abs_url(u):
    ...
    for r in results:
        ...
        if not url:
            continue
        r["url"] = url
        for k in ("thumbnail_src", "img_src", "source"):
            if r.get(k):
                r[k] = _abs_url(r[k]) or r[k]
        clean.append(r)
