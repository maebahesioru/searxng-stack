...
def extract_json_data(text: str) -> dict[str, t.Any]:
    ...
    try:
        ...
        raise SearxEngineResponseException("no script block in the response") from exc
    if not text:
        raise SearxEngineResponseException("can't find JS/JSON data in the given text")
    start = text.find("data: [{")
    if start < 0:
        # the page is an error/blocked page (429 etc.) - no JSON data to parse
        raise SearxEngineResponseException("no JSON data in the response (blocked page?)")
    end = text.rindex("}}]")
