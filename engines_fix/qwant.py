...
def response(resp: "SXNG_Response") -> EngineResults:
    ...
    if qwant_categ == "web":
        ...
        # in older API responses; the result items were directly in the list
        # result["items"].  Since 2025 the API returns the mainline structure
        # for ALL categories (rows typed "web"), so handle both shapes.
        items = data.get("result", {}).get("items", [])
        if isinstance(items, dict):
            mainline = items.get("mainline", [])
        elif isinstance(items, list):
            mainline = [{"type": qwant_categ, "items": items}]
        else:
            mainline = []
    ...
    for row in mainline:
        ...
        if qwant_categ == "web":
            # keep the original per-row type filtering for web queries
            if mainline_type != "web":
                continue
        # for news/images/videos the API now reports rows typed "web"; the
        # items of every row are parsed as the requested category below.

        if qwant_categ == "web" and mainline_type == "ads":
            # ignore adds
            continue
        ...
        for item in mainline_items:
            ...
            if qwant_categ == "news" or (qwant_categ == "web" and mainline_type == "news"):
            ...
            elif qwant_categ == "images" or (qwant_categ == "web" and mainline_type == "images"):
                res.add(
                    res.types.LegacyResult(
                        title=title,
                        url=res_url,
                        template="images.html",
                        thumbnail_src=item.get("thumbnail") or "",
                        img_src=item.get("media") or "",
                        resolution=f"{item.get('width', '?')} x {item.get('height', '?')}",
                        img_format=item.get("thumb_type"),
                    )
                )

            elif qwant_categ == "videos" or (qwant_categ == "web" and mainline_type == "videos"):
                ...
                length = timedelta(milliseconds=(item.get("duration") or 0))
